import { initializeApp } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-app.js";
    import {
        ...
        signOut, onAuthStateChanged, updateEmail, GoogleAuthProvider, signInWithRedirect, getRedirectResult
    ...
    window.openInvitationModal = (invId) => {
        ...
        contentBox.innerHTML = `
            ...
            <div style="font-size:0.9rem; display:flex; flex-direction:column; gap:8px;">
                <div><strong>Datum & Uhrzeit:</strong> ${inv.datetime}</div>
                <div><strong>Erstellt von:</strong> ${inv.createdBy}</div>
                ...
                <div>
                    <a href="${inv.mapsUrl}" target="_blank" style="color:var(--primary); font-weight:500; display:flex; align-items:center; gap:4px; text-decoration:underline;">
    ...
    window.startEditingInvitation = (invId) => {
        ...
        document.getElementById('datetime-picker')._flatpickr.setDate(inv.datetime, true);
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            if (userDoc.exists()) {
                ...
                roleBadge.className = currentUserData.role === 'admin' ? 'badge-admin' : 'badge-user';

                if (currentUserData.role === 'admin') {
                    document.getElementById('admin-section').classList.remove('hidden');
                    document.getElementById('admin-tags-section').classList.remove('hidden');
                    document.querySelectorAll('.admin-only-element').forEach(el => el.classList.remove('hidden'));
                } else {
                    document.getElementById('admin-section').classList.add('hidden');
                    document.getElementById('admin-tags-section').classList.add('hidden');
                    document.querySelectorAll('.admin-only-element').forEach(el => el.classList.add('hidden'));
                }
            }
            initApp();
        } else {
        ...
        try {
            ...
            for (const user of selectedUsers) {
                let personalizedMessage = messageTemplate
                    ...
                    .replace(/{{sender}}/g, currentUserData.name)
                ...
                await emailjs.send("service_oxlrwkl", "template_rl8z3ur", {
                    ...
                    reply_to: currentUserData.email
                });
            }

            alert('Einladungen erfolgreich und personalisiert versendet!');
        } catch (err) {
            alert('Fehler beim Versenden der E-Mails: ' + JSON.stringify(err));
        }
    }
    ...
    window.updateAndResendInvitation = async (invId) => {
        ...
        try {
            await updateDoc(doc(db, "invitations", invId), {
                recipients: recipientEmails, datetime, details: messageTemplate, updatedAt: serverTimestamp()
            });
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                            <div style="display:flex; flex-direction:column; gap:4px;">
                                <div style="font-size:0.9rem; display:flex; align-items:center; gap:6px;">
                                    ...
                                    <strong>Datum: ${inv.datetime}</strong>
        ...
        usersList.forEach(u => {
            if (u.email !== auth.currentUser.email) {
                ...
                dmItem.innerHTML = `
                    ...
                    <div class="room-meta">
                        <div class="room-name">${u.name}</div>
    ...
    window.switchChatRoom = (roomId, title, subtitle, avatarHTML) => {
        ...
        document.querySelectorAll('.chat-room-item').forEach(el => {
            if (el.onclick && el.onclick.toString().includes(roomId)) {
    ...
    function setupTagSettingsPanel(tagName) {
        ...
        usersList.forEach(u => {
            ...
            membersHtml += `<label style="display:block; margin-bottom:4px; font-weight:normal; font-size:0.85rem;">
                <input type="checkbox" ${hasTag ? 'checked' : ''} onchange="window.toggleUserTag('${u.id}', '${tagName}', this.checked)"> ${u.name} (${u.email})
            </label>`;
        });
        membersHtml += '</div></div>';

        if (tagObj) {
            membersHtml += `<div style="border-top:1px solid var(--border); pt:8px; display:flex; justify-content:flex-end;">
    ...
    function renderAdminUsers() {
        ...
        usersList.forEach(u => {
            ...
            div.innerHTML = `
                <div style="font-size:0.85rem;"><strong>${u.name}</strong> (${u.email})<br>Rolle: ${u.role}</div>
                ${tagsCheckboxes}
