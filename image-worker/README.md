...
Der Worker lädt Blog-, Profil- und Chatbilder zu ImgBB hoch und versendet E-Mail-Benachrichtigungen, ohne sensible Schlüssel an den Browser auszuliefern.
...
Der produktive Worker ist unter `https://huettenportal-image-worker.j-s-schulze.workers.dev` erreichbar. Die App verwendet `/upload` für Bilder und `/notify` für E-Mail-Benachrichtigungen. Für weitere Origins `ALLOWED_ORIGIN` in `wrangler.toml` anpassen und erneut deployen.
